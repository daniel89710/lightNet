...
void *detect_in_thread(void *ptr)
{
    while (!custom_atomic_load_int(&flag_exit)) {
	  ...
	  //	  image resized_mask = resize_image(mask, cv_images[demo_index]->w, cv_images[demo_index]->h); //Attention : Bilinear Interpolation
	  //	  blend_images_cv(cv_images[demo_index], 1.0, resized_mask, 0.5);
    ...
    int benchmark, int benchmark_layers)
{
    ...
    while(1){
		...
		lseg = &lk;
		printf(" Segmentation layer: %d - type = %d \n", k, l.type);
		cuda_pull_array(lseg->output_gpu, lseg->output, lseg->w * lseg->h * lseg->c);
		unsigned char *argmax = apply_argmax(lseg->output, lseg->w, lseg->h, lseg->c);
		image mask = argmax2mask(argmax, lseg->w, lseg->h, lseg->map);
		char buff[256];
		sprintf(buff, "mask%d", k);		
		show_image(mask, buff);
		//	      image resized_mask = resize_image(mask, get_width_mat(in_img), get_height_mat(in_img));
		//	      blend_images_cv_v3(in_img, 1.0, resized_mask, 0.5);
		free(argmax);
		free_image(mask);
	    ...
	    /*
	    if (lseg) {
	      cuda_pull_array(lseg->output_gpu, lseg->output, lseg->w * lseg->h);
	      unsigned char *argmax = apply_argmax(lseg->output, lseg->w, lseg->h, lseg->c);
	      image mask = argmax2mask(argmax, lseg->w, lseg->h, lseg->map);
