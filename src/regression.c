...
extern float validate_regression_iou(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, int letter_box, network *existing_net, sqlite3 *db);
...
void train_regression(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dont_show, int calc_map, float thresh, float iou_thresh, int mjpeg_port, int show_imgs, int benchmark_layers, char* chart_path)
{
    ...
    for (k = 0; k < net.n; ++k) {
        ...
        if (lk.type == LOGXENT) {
            l = lk;
            printf(" Logistic layer: %d - type = %d \n", k, l.type);
	...
	insert_loss(db, iteration, train_images_num, avg_loss, loss, get_current_rate(net), net.batch, ngpus);
        ...
        if (calc_map && (iteration >= next_map_calc || iteration == net.max_batches)) {
            ...
            iter_map = iteration;
            mean_average_precision = validate_regression_iou(datacfg, cfgfile, weightfile, thresh, iou_thresh, 0, net.letter_box, &net_map, db);// &net_combined);
...
void print_regression_detections(FILE **fps, char *id, detection *dets, int total, int classes, int w, int h)
{
    int i, j;
    for (i = 0; i < total; ++i) {
...
void validate_regression(char *datacfg, char *cfgfile, char *weightfile, char *outfile)
{
    int j;
    list *options = read_data_cfg(datacfg);
    ...
    for (i = nthreads; i < m + nthreads; i += nthreads) {
        ...
        for (t = 0; t < nthreads && i + t - nthreads < m; ++t) {
            ...
            if (coco) {
                ...
                print_regression_detections(fps, id, dets, nboxes, classes, w, h);
            }
...
void validate_regression_recall(char *datacfg, char *cfgfile, char *weightfile)
{
    network net = parse_network_cfg_custom(cfgfile, 1, 1);    // set batch=1
    if (weightfile) {
        load_weights(&net, weightfile);
...
float validate_regression_iou(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, int letter_box, network *existing_net, sqlite3 *db)
{
    int j;
    list *options = read_data_cfg(datacfg);
    char *valid_images = option_find_str(options, "valid", "data/train.txt");
...
float uncertain_regression_iou(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, int letter_box, network *existing_net, sqlite3 *db)
{
    int j;
    list *options = read_data_cfg(datacfg);
...
static void calc_anchors(char *datacfg, int num_of_clusters, int width, int height, int show)
{
    ...
    if (width < 0 || height < 0) {
        printf("Usage: darknet regression calc_anchors data/voc.data -num_of_clusters 9 -width 416 -height 416 \n");
...
void test_regression(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh,
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers)
{
	...
	for (k = 0; k < net.n; ++k) {
	    ...
	    if (lk.type == LOGXENT) {	    
                l = lk;
                printf(" Logigistic layer: %d - type = %d \n", k, l.type);
		cuda_pull_array(l.output_gpu, l.output, l.w * l.h * l.c);
		image m = make_image(l.w, l.h, l.c);
		m.data = l.output;
		save_image_png(m, "depth_prediction");
		image resized_mask = resize_image(m, im.w, im.h); //Attention : Bilinear Interpolation
...
void merge_regression(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh,
...
void draw_object(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, int dont_show, int it_num,
    int letter_box, int benchmark_layers)
{
    printf(" ./darknet regression draw ... can't be used without OpenCV and CUDA! \n");
    getchar();
}
#endif // defined(OPENCV) && defined(GPU)

void run_regression(int argc, char **argv)
{
    ...
    if (0 == strcmp(argv[2], "test")) test_regression(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels, outfile, letter_box, benchmark_layers);
    else if (0 == strcmp(argv[2], "train")) train_regression(datacfg, cfg, weights, gpus, ngpus, clear, dont_show, calc_map, thresh, iou_thresh, mjpeg_port, show_imgs, benchmark_layers, chart_path);
    else if (0 == strcmp(argv[2], "valid")) validate_regression(datacfg, cfg, weights, outfile);
    else if (0 == strcmp(argv[2], "recall")) validate_regression_recall(datacfg, cfg, weights);
    else if (0 == strcmp(argv[2], "iou")) validate_regression_iou(datacfg, cfg, weights, thresh, iou_thresh, map_points, letter_box, NULL, NULL);
    else if (0 == strcmp(argv[2], "uncertain")) uncertain_regression_iou(datacfg, cfg, weights, thresh, iou_thresh, map_points, letter_box, NULL, NULL);    
    else if (0 == strcmp(argv[2], "merge")) merge_regression(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels, outfile, letter_box, benchmark_layers, merge_cfg, merge_weights, from);
