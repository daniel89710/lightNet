...
extern "C" image mat_to_image_with_scale(cv::Mat mat, int scale)
{
    int w = mat.cols;
    int h = mat.rows;
    int c = mat.channels();
    ...
    for (int y = 0; y < h; ++y) {
        for (int k = 0; k < c; ++k) {
	      ...
	      if (data[y*step + x*c + k] > 10) {
		im.data[k*w*h + y*w + x] = (data[y*step + x*c + k]-10) / ((float)scale);
	      } else {
		im.data[k*w*h + y*w + x] = 0.0;
	      }
	      im.data[k*w*h + y*w + x] = im.data[k*w*h + y*w + x] > 1.0 ? 1.0 : im.data[k*w*h + y*w + x];
	      if (im.data[k*w*h + y*w + x]) {
		im.data[k*w*h + y*w + x] = 1.0 - im.data[k*w*h + y*w + x];
	      }
            }
        }
    }
...
extern "C" image mask_data_augmentation(mat_cv* mat, int w, int h,
    int pleft, int ptop, int swidth, int sheight, int flip,
    float dhue, float dsat, float dexp,
    int gaussian_noise, int blur, int num_boxes, int truth_size, float *truth)
{
    ...
    try {
        ...
        if (src_rect.x == 0 && src_rect.y == 0 && src_rect.size() == img.size()) {
            cv::resize(img, sized, cv::Size(w, h), 0, 0, cv::INTER_NEAREST);
        }
        else {
            cv::Mat cropped(src_rect.size(), img.type());
            //cropped.setTo(cv::Scalar::all(0));
        ...
        cerr << "OpenCV can't augment mask: " << w << " x " << h << " \n";
        out = mat_to_image(*(cv::Mat*)mat);
    }
    return out;
}

extern "C" image get_depth_data_augmentation(mat_cv* mat, int w, int h,
    int pleft, int ptop, int swidth, int sheight, int flip,
    float dhue, float dsat, float dexp,
					     int gaussian_noise, int blur, int num_boxes, int truth_size, float *truth, int scale)
{
    image out;
    try {
        cv::Mat img = *(cv::Mat *)mat;
        ...
        // Mat -> image
        out = mat_to_image_with_scale(sized, scale);
    }
    catch (...) {
        cerr << "OpenCV can't augment mask: " << w << " x " << h << " \n";
        out = mat_to_image(*(cv::Mat*)mat);
...
extern "C" void blend_images_cv_v3(mat_cv *mat, float alpha, image old_img, float beta)
{
  cv::Mat &new_mat = *(cv::Mat*)mat;
  cv::Mat old_mat(cv::Size(old_img.w, old_img.h), CV_32FC(old_img.c), old_img.data);
  cv::addWeighted(new_mat, alpha, old_mat, beta, 0.0, new_mat);
}
