...
void
quantize_weights_per_tensor(layer *l, unsigned char b)
{

  int out_c = l->c;
  int in_c = l->n;
  int kernel = l->size * l->size;
  
  const int window = 4;
  const int nonzeros = 2;
  int crs = kernel * in_c;
#ifdef GPU
  pull_convolutional_layer(*l);
#endif
  ...
  for (ki = 0; ki < out_c; ++ki) {
    for (rsi = 0; rsi < kernel; ++rsi) {
      for (ci = 0; ci < in_c; ++ci) {
	int index = getIndex(ki, ci, rsi, kernel, crs);
	max_weight = MAX(max_weight, fabs(l->weights[index]));
      }
    }
  }
  float max_value = pow(2, b-1) - 1; 
  float scale = (max_value) / max_weight;
  printf("Max Value %f, Scale %f@%dbit\n", max_weight, scale, b);

  for (ki = 0; ki < out_c; ++ki) {
    for (rsi = 0; rsi < kernel; ++rsi) {
      for (ci = 0; ci < in_c; ++ci) {
	int index = getIndex(ki, ci, rsi, kernel, crs);
	int q_w = round(l->weights[index] * scale);
	q_w = (q_w > max_value) ? max_value : q_w;
	q_w = (q_w < (max_value * (-1))) ? (max_value * (-1)) : q_w;
	float de_w = q_w / scale;
	l->weights[index] = de_w;
      }
    }
  }  
#ifdef GPU
...
void quantize_network(network *net)
{
  int i;
  for(i = 0; i < net->n; i++) {
    layer l = net->layers[i];
    if (l.type == CONVOLUTIONAL && l.quant) {
      //Todo : Per-channel Qunatization
      printf("Quantize Conv%d for weights ... ", i);
      quantize_weights_per_tensor(&(net->layers[i]), 8);
    }
  }
 }

float
get_max_value(float *output, int num)
{
  int i;
  float max_value = 0.0; 
  for (i = 0; i < num; i++) {
    max_value = MAX(max_value, fabs(output[i]));
  }
  return max_value;
}

void
calibrate_network(network *net, int id)
{
 int i;
  for(i = 0; i < net->n; i++) {
    layer *l = &net->layers[i];
    cuda_pull_array(l->output_gpu, l->output, l->outputs * l->batch);
    float value = get_max_value(l->output, l->outputs * l->batch);
    l->statics[id] = value;
  } 
}
...
/**
 * @brief Calculate the index of an element in a multidimensional array.
 *
 * This function computes the index of an element in a multidimensional array
 * based on the given parameters. The array is assumed to have dimensions of
 * kernel, kernel, and crs.
 * This source code based on TensorRT samples.
 * https://github.com/NVIDIA/TensorRT/blob/release/8.6/samples/common/sampleUtils.cpp
 *
 * @param ki The index along the first dimension (0 <= ki < kernel).
 * @param ci The index along the second dimension (0 <= ci < kernel).
 * @param rsi The index along the third dimension (0 <= rsi < crs).
 * @param kernel The size of the first and second dimensions.
 * @param crs The size of the third dimension.
 * @return The computed index in the multidimensional array.
 */

int
getIndex(int32_t ki, int32_t ci, int32_t rsi, int kernel, int crs)
{
  //[out_c][in_c][kernel]
  return ki * crs + ci * kernel + rsi;
}

/**
 * @brief Sparsify the given layer using a 2/4 fine-grained structured sparsity technique.
 * This source code based on TensorRT samples.
 * https://github.com/NVIDIA/TensorRT/blob/release/8.6/samples/common/sampleUtils.cpp
 *
 * This function applies a 2/4 fine-grained structured sparsity technique to
 * the provided layer 'l'. The objective of this function is to increase the
 * sparsity of the layer by setting a portion of its weights to zero, which
 * can lead to reduced memory footprint and computational complexity.
 *
 * @param l A pointer to the layer to be sparsified.
 */
void
sparsify_layer_by_2_4_fine_grained(layer *l)
{
  int out_c = l->c;
  int in_c = l->n;
  int kernel = l->size * l->size;
  ...
  for (ki = 0; ki < out_c; ++ki) {
    for (rsi = 0; rsi < kernel; ++rsi) {
      ...
      for (ci = 0; ci < in_c; ++ci) {
	int index = getIndex(ki, ci, rsi, kernel, crs);
	buffer[w] = l->weights[index];
	indexes[w] = index;
	if (++w == window) {
	  w = 0;
	  for (i = 0; i < window; i++) {
	    for (j = i+1; j < window; j++) {	    
	      if (fabs(buffer[i]) < fabs(buffer[j])) {
		float b_tmp = buffer[i];
		buffer[i] = buffer[j];
		buffer[j] = b_tmp;
		int i_tmp = indexes[i];
		indexes[i] = indexes[j];
	  ...
	  for (i = 0; i < nonzeros; i++) {
	    l->sparse_mask[indexes[i]] = 1.0;
	    indexes[i] = -1;
	    buffer[i] = -1.0;
	  }	  
	  for (i = nonzeros; i < window; i++) {
	    l->weights[indexes[i]] = 0.0;
	    l->sparse_mask[indexes[i]] = 0.0;	    
	    indexes[i] = -1;
	    buffer[i] = -1.0;
	  }
	}
...
#ifdef GPU
  push_convolutional_layer(*l);
  cuda_push_array(l->sparse_mask_gpu, l->sparse_mask, l->nweights);  
#endif
 free(buffer);
 free(indexes);
}
...
/**
 * @brief Get the number of zero weights in the given layer.
 *
 * This function calculates the number of zero weights (sparsity) in the given layer.
 *
 * @param l The layer to calculate sparsity.
 * @return The number of zero weights in the layer.
 */
int
get_num_sparsity(layer l)
{
  int n;
  int nSparsity = 0;
  for (n = 0; n < l.nweights; n++) {
    if (l.weights[n] == 0.0) {
      nSparsity++;
    }
  }
...
/**
 * @brief Sparsify weights in the given network.
 *
 * This function sparsifies the weights in the given network by applying the specified sparsity algorithms
 * to the layers.
 *
 * @param net The network to sparsify.
 */
void
sparsify_weights(network *net)
{
  ...
  for(i = 0; i < net->n; i++) {
    ...
    if (l.type == CONVOLUTIONAL && l.share_layer == NULL) {
      if (l.sparse) {
	fprintf(stderr, "Sparse Conv%d (%dx%dx%d)", i, l.c, l.n, l.size);
	sparsify_layer_by_2_4_fine_grained(&(net->layers[i]));
	int nSparsity = get_num_sparsity(l);
	float sparsity = (float)nSparsity/(float)l.nweights;
	fprintf(stderr, " : %f\n", sparsity);
      }
    }
  }
...
/**
 * @brief Struct Weight_info is used to store weight information.
 */
typedef struct weight_info {
  int index;    /**< @brief Index */
  float value;  /**< @brief Weight value */
} Weight_info;


/**
 * @brief The prune_conv_channel function prunes the channels of the given convolution layer.
 * @param conv Pointer to the convolution layer structure.
 * @param mask Pointer to the mask array that will store the pruning information.
 * @param prev_mask Pointer to the mask array of the previous layer.
 */
void prune_conv_channel(layer *conv, unsigned char*mask, unsigned char *prev_mask)
{
  int in_c = conv->c;
  int out_c = conv->n;
  int groups = conv->groups;
  int kernel = conv->size * conv->size; 
  int crs = kernel * in_c;
  int k, cin, cout, i, j;

  Weight_info *w_info = (Weight_info *)malloc(sizeof(Weight_info) * out_c);
  for (cout = 0; cout < out_c; ++cout) {
    float sum = 0.0;
    for (cin = 0; cin < in_c; ++cin) {  
      for (k = 0; k < kernel; ++k) {
	int index = getIndex(cout, cin, k, kernel, crs);
	sum += fabs(conv->weights[index]);
      }
    }
    w_info[cout].index = cout;
    w_info[cout].value = sum;        
  }
  for (i = 0; i < out_c; i++) {
    for (j = i+1; j < out_c; j++) {
      if (w_info[i].value > w_info[j].value) {
	int tmp_index = w_info[i].index;
	float tmp_value = w_info[i].value;
	w_info[i].index = w_info[j].index;
	w_info[i].value = w_info[j].value;
	w_info[j].index = tmp_index;
	w_info[j].value = tmp_value;	
      }
    }
  }
  int prune_indexes = (int)(conv->pruning * out_c);
  
  //prune_indexes = 0;
  int remained = out_c - prune_indexes;  
  for (i = 0; i < prune_indexes; i++) {
    int index = w_info[i].index;
    mask[index] = 0;
  }
  conv->n = remained;
  if (prev_mask) {
    conv->c =count_mask(prev_mask, in_c);
  }
  fprintf(stderr, "mask[%d/%d]=", (remained),out_c);
  for (i = 0; i < out_c; i++) {
    fprintf(stderr, "%d,", mask[i]);
  }
  fprintf(stderr, "\n");
  //Prune Channel
  float *old_biases, *old_scales, *old_rolling_mean, *old_rolling_variance, *old_weights;
  old_biases = conv->biases;
  old_weights = conv->weights;
  //Todo Group Conv;
  conv->nweights = remained * (conv->c /groups) * kernel;
  conv->weights = (float*)xcalloc(conv->nweights, sizeof(float));  
  conv->biases = (float*)xcalloc(remained, sizeof(float));  
  if (conv->batch_normalize) {
    old_scales = conv->scales;
    old_rolling_mean = conv->rolling_mean;
    old_rolling_variance = conv->rolling_variance;
    conv->scales = (float*)xcalloc(remained, sizeof(float));
    conv->rolling_mean = (float*)xcalloc(remained, sizeof(float));  
    conv->rolling_variance = (float*)xcalloc(remained, sizeof(float));  
  }
  int count = 0;
  int w_count = 0;  
  crs = kernel * in_c;
  for (cout = 0; cout < out_c; cout++) {
    if (mask[cout]) {
      conv->biases[count] = old_biases[cout];
      if (conv->batch_normalize) {
	conv->scales[count] = old_scales[cout];
	conv->rolling_mean[count] = old_rolling_mean[cout];
	conv->rolling_variance[count] = old_rolling_variance[cout];		
      }      
      for (cin = 0; cin < in_c; ++cin) {
	if (prev_mask[cin]) {
	  for (k = 0; k < kernel; ++k) {
	    int index = getIndex(cout, cin, k, kernel, crs);
	    conv->weights[w_count++] = old_weights[index];
	  }
	} else {
	  int test = 0;
	}
  ...
  if (conv->batch_normalize) {
    free(old_scales);
    free(old_rolling_mean);
    free(old_rolling_variance);  
  }
...
/**
 * @brief The count_mask function counts the number of non-zero elements in a mask array.
 * @param mask Pointer to the mask array.
 * @param num The number of elements in the mask array.
 * @return The number of non-zero elements in the mask array.
 */
int
count_mask(unsigned char *mask, int num)
{
  int n;
  int count = 0;
  for (n = 0; n < num; n++) {
...
/**
 * @brief The count_unmask function counts the number of zero elements in a mask array.
 * @param mask Pointer to the mask array.
 * @param num The number of elements in the mask array.
 * @return The number of zero elements in the mask array.
 */
int
count_unmask(unsigned char *mask, int num)
{
  int n;
  int count = 0;
  for (n = 0; n < num; n++) {
...
/**
 * @brief The or_mask function performs bitwise OR between two mask arrays.
 * @param dst Pointer to the destination mask array.
 * @param src Pointer to the source mask array.
 * @param num The number of elements in the mask arrays.
 */
void
or_mask(unsigned char *dst, unsigned char *src, int num)
{
  int n;
  int count = 0;
  for (n = 0; n < num; n++) {
...
/**
 * @brief The and_mask function performs bitwise AND between two mask arrays.
 * @param dst Pointer to the destination mask array.
 * @param src Pointer to the source mask array.
 * @param num The number of elements in the mask arrays.
 */
void
and_mask(unsigned char *dst, unsigned char *src, int num)
{
  int n;
  int count = 0;
  for (n = 0; n < num; n++) {
...
/**
 * @brief Prune weights in the given network.
 *
 * This function prunes the weights in the given network by applying the specified pruning algorithms
 * to the layers.
 *
 * @param net The network to prune.
 */
void
prune_weights(network *net)
{
  int i, j, n;
  unsigned char **prune_masks = (unsigned char**)malloc(sizeof(unsigned char*) * net->n);
  int *mask_counts  = (int *)malloc(sizeof(int *) * net->n);
  for(i = 0; i < net->n; i++) { 
    layer l = net->layers[i];
    prune_masks[i] = NULL;
    prune_masks[i] = (unsigned char*)malloc(sizeof(unsigned char) * l.out_c);    
    for (j = 0; j < l.out_c; j++) {
      (prune_masks[i])[j] = 1;
    }
    mask_counts[i] = l.out_c;
  }
  for(i = 0; i < net->n; i++) {
    layer l = net->layers[i];
    unsigned char *prev_mask = NULL;
    if (i > 0) {
      prev_mask = prune_masks[i-1];
    }    
    if (l.type == CONVOLUTIONAL && l.share_layer == NULL) {      
      if (l.pruning > 0.0 || (prev_mask && count_unmask(prev_mask, l.c))) {
	fprintf(stderr, "Prune Conv%d (%dx%dx%d)\n", i, l.c, l.n, l.size);
	prune_conv_channel(&net->layers[i], prune_masks[i], prev_mask);	
      }
    } else if (l.type == ROUTE) {
      int stride = 0;
      for (n = 0; n < l.n; n++) {
	int index = l.input_layers[n];
	and_mask(&((prune_masks[i])[stride]), prune_masks[index], mask_counts[index]);
	stride += mask_counts[index];
      }
    } else if (l.type == MAXPOOL) {
      if (i != 0) {
	and_mask(prune_masks[i], prune_masks[i-1], l.c);
      }
...
/**
 * @brief Debug the weights in the given convolutional layer.
 *
 * This function prints the weights and other information for the given convolutional layer.
 *
 * @param conv The convolutional layer to debug.
 */
void debug_conv(layer *conv)
{
  int in_c = conv->c;
  int out_c = conv->n;
  int kernel = conv->size * conv->size; 
  ...
  fprintf(stderr, "biases[%d]=",out_c);
  for (cout = 0; cout < out_c; ++cout) {
    fprintf(stderr, "%f,", conv->biases[cout]);
  }
  if (conv->batch_normalize) {
    fprintf(stderr, "\n\nscales[%d]=",out_c);
    for (cout = 0; cout < out_c; ++cout) {
      fprintf(stderr, "%f,", conv->scales[cout]);
    }
    fprintf(stderr, "\n\nrolling_mean[%d]=",out_c);
    for (cout = 0; cout < out_c; ++cout) {
      fprintf(stderr, "%f,", conv->rolling_mean[cout]);
    }
    fprintf(stderr, "\n\nrolling_variance[%d]=",out_c);
    for (cout = 0; cout < out_c; ++cout) {
      fprintf(stderr, "%f,", conv->rolling_variance[cout]);
    }        
  }
  fprintf(stderr, "\n");
  fprintf(stderr, "\n\nweights[%dx%dx%dx%d]=",out_c, in_c, conv->size, conv->size);
  for (cout = 0; cout < out_c; ++cout) {
    for (cin = 0; cin < in_c; ++cin) {
      fprintf(stderr, "{");
      for (k = 0; k < kernel; ++k) {
	int index = getIndex(cout, cin, k, kernel, crs);
	fprintf(stderr, "%f,", conv->weights[index]);
      }
      fprintf(stderr, "}");
    }
    fprintf(stderr, "\n");
...
/**
 * @brief Debug the weights in the given network.
 *
 * This function prints the weights and other information for each layer in the given network.
 *
 * @param net The network to debug.
 */
void
debug_weights(network *net)
{
  int i;
  for(i = 0; i < net->n; i++) {
    layer l = net->layers[i];
    if (l.type == CONVOLUTIONAL) {
      ...
      fprintf(stderr, "\n\n###Debug Conv%d (%dx%dx%d)\n", i, l.c, l.n, l.size);
      debug_conv(&net->layers[i]);	
    }
  }
}
