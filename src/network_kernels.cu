...
void forward_network_gpu(network net, network_state state)
{
	...
	if (l.quant && l.max_float > 0.0) {
	  int b = 8;
	  int max_value = (int)pow(2, b-1) - 1; 
	  float scale = (max_value) / l.max_float;
	  //      cuda_pull_array(l.output_gpu, l.output, l.outputs * l.batch);
	  apply_fake_quantization_gpu(l.outputs*l.batch, l.output_gpu, scale, max_value);
	  //      cuda_pull_array(l.output_gpu, l.output, l.outputs * l.batch);
	  int test = 1;
	}
        if (net.benchmark_layers) {
