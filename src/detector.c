...
extern char **get_random_paths(char **paths, int n, int m);
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dont_show, int calc_map, float thresh, float iou_thresh, int mjpeg_port, int show_imgs, int benchmark_layers, char* chart_path)
{
	...
	if (calc_map && (iteration >= next_map_calc || iteration == net.max_batches)) {
	    ...
	    *net_map.cur_iteration = *net.cur_iteration;
	    ...
	    //            mean_average_precision = validate_detector_map(datacfg, cfgfile, weightfile, thresh, iou_thresh, 0, net.letter_box, &net_map, db);// &net_combined);
            mean_average_precision = validate_detector_map(datacfg, cfgfile, buff, thresh, iou_thresh, 0, net.letter_box, NULL, db);// &net_combined);	    
...
float validate_detector_map(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, int letter_box, network *existing_net, sqlite3 *db)
{
    int j;
    list *options = read_data_cfg(datacfg);
    char *valid_images = option_find_str(options, "valid", "data/train.txt");
	...
	Eval_detection eval;
	eval.ap = avg_precision;
	eval.iou = avg_iou_per_class[i];
	eval.tp = tp_for_thresh_per_class[i];
	eval.fn = truth_classes_count[i] - tp_for_thresh_per_class[i];
	eval.fp = fp_for_thresh_per_class[i];
	insert_eval_detection(db, eval, "val", names[i], iteration, train_images_num, batch, 1);
    }
...
float validate_detector_per_image(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, int letter_box, network *existing_net, sqlite3 *db)
{
    int j;
    list *options = read_data_cfg(datacfg);
    ...
    create_eval_detection(db);
    create_eval_detection_per_image(db);            
    //int initial_batch;
    if (existing_net) {
        char *train_images = option_find_str(options, "train", "data/train.txt");
        valid_images = option_find_str(options, "valid", train_images);
    ...
    for (i = nthreads; i < m + nthreads; i += nthreads) {
	  ...
	  int j;
	  for (j = 0; j < classes; j++) {
	    avg_iou_per_class[j] = 0.0;
	    tp_for_thresh_per_class[j] = 0;
	    fp_for_thresh_per_class[j] = 0;
	    truth_classes_count[j] = 0;	    
	  }
	  const int image_index = i + t - nthreads;	   
	  char *path = paths[image_index];
	  char *id = basecfg(path);
	  ...
	  for (j = 0; j < classes; j++) {
	    float iou = -1.0;
	    if (tp_for_thresh_per_class[j]+fp_for_thresh_per_class[j]) {
	      iou = avg_iou_per_class[j]/ (tp_for_thresh_per_class[j]+fp_for_thresh_per_class[j]);
	    }
	    printf("##%s [%s]->TP=%d, FP=%d, FN=%d, IOU=%f\n",
		   path,
		   names[j],
		   tp_for_thresh_per_class[j],
		   fp_for_thresh_per_class[j],
		   truth_classes_count[j]-tp_for_thresh_per_class[j],
		   iou
		   );
	    Eval_segmentation eval;
	    eval.tp = tp_for_thresh_per_class[j];
	    eval.fn = truth_classes_count[j]-tp_for_thresh_per_class[j];
	    eval.fp = fp_for_thresh_per_class[j];
	    insert_eval_detection_per_image(db, eval, "train", names[j], iteration, train_images_num, batch, 1, path);	    
	  }
        }
    }
...
void sparse_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh,
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    ...
    sparsify_weights(&net);

    char buff[256];
    sprintf(buff, "%s/%s.sparse.weights", backup_directory, base);
    save_weights(net, buff);
...
void prune_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh,
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers)
{
    gpu_index = -1;
    list *options = read_data_cfg(datacfg);
    ...
    sprintf(buff, "%s/%s.sparse.weights", backup_directory, base);
    save_weights(net, buff);
    fprintf(stderr, "Complete to save %s\n", buff);
    sprintf(buff, "%s.pruned.cfg", base);    
    write_cfg(net, buff);
    // free memory
...
void debug_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh,
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers)
{
    gpu_index = -1;
    list *options = read_data_cfg(datacfg);
...
void validate_for_sensitivity_analysis(char *datacfg, char *cfgfile, char *weightfile, float thresh, float iou_thresh, int letter_box, network net, int index, sqlite3 *db)
{
  //re-load weights
  if (weightfile) {
    load_weights(&net, weightfile);
  ...
  *net.cur_iteration = index;  
  if (index != -1) {
    net.layers[index].sparse = 1;
    //set iteration
    net.layers[index].sparse_mask = (float *)malloc(sizeof(float) * net.layers[index].nweights);
#ifdef GPU
    net.layers[index].sparse_mask_gpu = NULL;
    net.layers[index].sparse_mask_gpu = (float *)cuda_make_array(net.layers[index].sparse_mask, net.layers[index].nweights);
#endif
    //sparse a layer
    sparsify_weights(&net);
  }
  ...
  for(i = 0; i < net.n; i++) {
    ...
    if (l.type == CONVOLUTIONAL) {
      ...
      //fprintf(stderr, " : %f\n", sparsity);
      total_weights += l.nweights;
      total_sparses += nSparsity;
    }
  }
  fprintf(stderr, "Total Sparsity : %f\n", total_sparses/(float)total_weights);  
  */
  validate_detector_map(datacfg, cfgfile, weightfile, thresh, iou_thresh, 0, letter_box, &net, db);

  if (index != -1) {  
    net.layers[index].sparse = 0; 
#ifdef GPU 
    cuda_free(net.layers[index].sparse_mask_gpu);
    net.layers[index].sparse_mask_gpu = NULL;
#endif
    free(net.layers[index].sparse_mask);
    net.layers[index].sparse_mask = NULL;
  }
}

void calc_sensitivity(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh,
    float iou_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    int names_size = 0;
    ...
    validate_for_sensitivity_analysis(datacfg, cfgfile, weightfile, thresh, iou_thresh, letter_box, net, -1, db);    
    for(i = 0; i < net.n; i++) {
      layer l = net.layers[i];
      if (l.type == CONVOLUTIONAL && l.share_layer == NULL) {
	fprintf(stderr, "Validate ...");
	validate_for_sensitivity_analysis(datacfg, cfgfile, weightfile, thresh, iou_thresh, letter_box, net, i, db);
      }
    }
...
void check_sparsity(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh,
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
...
void quantize_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh,
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    ...
    sprintf(buff, "%s/%s.quantized.weights", backup_directory, base);
    save_weights(net, buff);
...
void calibrate(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, int letter_box, network *existing_net, sqlite3 *db)
{
    int j;
    list *options = read_data_cfg(datacfg);
    char *calib_images = option_find_str(options, "train", "data/train.txt");
    ...
    if (existing_net) {
        char *train_images = option_find_str(options, "train", "data/train.txt");
        calib_images = option_find_str(options, "valid", train_images);
        net = *existing_net;
        remember_network_recurrent_state(*existing_net);
        free_network_recurrent_state(*existing_net);
    }
    ...
    list *plist = get_paths(calib_images);
    char **paths = (char **)list_to_array(plist);
    layer l = net.layers[net.n - 1];
    int k;
    for (k = 0; k < net.n; ++k) {
    ...
    int iter = 10;
    int n = iter+1;
    char **rand_paths = get_random_paths(paths, n, m);
    int i = 0;
    int t;
    ...
    for(k = 0; k < net.n; k++) {
      ...
      l->statics = (float *)malloc(sizeof(float) * iter);
    }
    
    
    for (t = 0; t < nthreads; ++t) {
        args.path = rand_paths[i + t];
        args.im = &buf[t];
        args.resized = &buf_resized[t];
        thr[t] = load_data_in_thread(args);
    }
    time_t start = time(0);
    for (i = nthreads; i < n; i += nthreads) {
      //fprintf(stderr, "\r%d", i);
        for (t = 0; t < nthreads && (i + t - nthreads) < m; ++t) {
            pthread_join(thr[t], 0);
            val[t] = buf[t];
        ...
        for (t = 0; t < nthreads && i + t - nthreads < m; ++t) {
            const int image_index = i + t - nthreads;
            char *path = rand_paths[image_index];
            char *id = basecfg(path);
            float *X = val_resized[t].data;
	    printf("%d:Calibrate ... %s\n", image_index, path);
            network_predict(net, X);
	    calibrate_network(&net, image_index);
        }
    }
    for(k = 0; k < net.n; k++) {
      layer *l = &net.layers[k];
      float sum = 0;
      printf("max_float@%d=",k);
      for (i = 0; i < iter; i++) {
	printf("%f,",l->statics[i]);
	sum += l->statics[i];
      }      
      l->max_float = sum/(float)iter;
      printf(" => %f\n", l->max_float);
      //l->quant = 1;
    }
    char buff[512];
    sprintf(buff, "%s-qat.cfg", base);    
    write_cfg(net, buff);   
    for(k = 0; k < net.n; k++) {
      layer *l = &net.layers[k];
      free(l->statics);
...
void run_detector(int argc, char **argv)
{
    ...
    if (0 == strcmp(argv[2], "test")) test_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels, outfile, letter_box, benchmark_layers);
    ...
    else if (0 == strcmp(argv[2], "uncertain")) validate_detector_per_image(datacfg, cfg, weights, thresh, iou_thresh, map_points, letter_box, NULL, NULL);    
      ...
      sparse_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels, outfile, letter_box, benchmark_layers);
    } else if (0 == strcmp(argv[2], "prune")) {
      prune_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels, outfile, letter_box, benchmark_layers);
    } else if (0 == strcmp(argv[2], "debug")) {
      debug_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels, outfile, letter_box, benchmark_layers);            
    } else if (0 == strcmp(argv[2], "sensitivity")) {
      calc_sensitivity(datacfg, cfg, weights, filename, thresh, iou_thresh, dont_show, ext_output, save_labels, outfile, letter_box, benchmark_layers);
    } else if (0 == strcmp(argv[2], "sparse_check")) {
      check_sparsity(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels, outfile, letter_box, benchmark_layers);
    } else if (0 == strcmp(argv[2], "quantize")) {
      quantize_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels, outfile, letter_box, benchmark_layers);
    } else if (0 == strcmp(argv[2], "calibrate")) {
      calibrate(datacfg, cfg, weights, thresh, iou_thresh, map_points, letter_box, NULL, NULL);
    }
