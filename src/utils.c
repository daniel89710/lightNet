...
void replace_image_to_mask(const char* input_path, char* output_path)
{
    find_replace(input_path, "/images/train2017/", "/labels/train2017/", output_path);    // COCO
    find_replace(output_path, "/images/val2017/", "/labels/val2017/", output_path);        // COCO
    find_replace(output_path, "/JPEGImages/", "/labels/", output_path);    // PascalVOC
    ...
    // replace only ext of files
    find_replace_extension(output_path, ".jpg", ".png", output_path);
    find_replace_extension(output_path, ".JPG", ".png", output_path); // error
    find_replace_extension(output_path, ".jpeg", ".png", output_path);
    find_replace_extension(output_path, ".JPEG", ".png", output_path);
    find_replace_extension(output_path, ".png", ".png", output_path);
    find_replace_extension(output_path, ".PNG", ".png", output_path);
    find_replace_extension(output_path, ".bmp", ".png", output_path);
    find_replace_extension(output_path, ".BMP", ".png", output_path);
    find_replace_extension(output_path, ".ppm", ".png", output_path);
    find_replace_extension(output_path, ".PPM", ".png", output_path);
    find_replace_extension(output_path, ".tiff", ".png", output_path);
    find_replace_extension(output_path, ".TIFF", ".png", output_path);

    // Check file ends with txt:
    if(strlen(output_path) > 4) {
        char *output_path_ext = output_path + strlen(output_path) - 4;
        if( strcmp(".png", output_path_ext) != 0){
            fprintf(stderr, "Failed to infer label file name (check image extension is supported): %s \n", output_path);
        }
    }else{
        fprintf(stderr, "Label file name is too short: %s \n", output_path);
...
float write_conv_layer(layer conv, FILE *fp, int index)
{

  float bflops = (2.0 * conv.nweights * conv.out_h*conv.out_w) / 1000000000.;
  fprintf(fp, "#L%d %dx%d %.3fGFLOPS\n", index, conv.w, conv.h, bflops);
  ...
  fprintf(fp, "stride=%d\n", conv.stride);
  fprintf(fp, "pad=%d\n", conv.pad);    
  fprintf(fp, "activation=%s\n", get_activation_string(conv.activation));
  if (conv.max_float) {
    fprintf(fp, "max_float=%f\n", conv.max_float);
  }
  if (conv.quant) {
    fprintf(fp, "quant=1\n");
  }  
...
float write_maxpool_layer(layer l, FILE *fp, int index)
{

  float bflops = (l.size*l.size *l.c * l.out_h*l.out_w) / 1000000000.;
  fprintf(fp, "#L%d %dx%d %.3fGFLOPS\n", index, l.w, l.h, bflops);
  fprintf(fp, "[maxpool]\n");
  fprintf(fp, "size=%d\n", l.size);
  fprintf(fp, "stride=%d\n", l.stride);  
  if (l.max_float) {
...
float write_route_layer(layer l, FILE *fp, int index)
{
  int n ;
  ...
  fprintf(fp, "layers=");    
  for (n = 0; n < l.n; n++) {
    int route_index = l.input_layers[n];
    fprintf(fp, "%d", route_index-index);
    if (n != (l.n - 1)) {
      fprintf(fp, ",");
    }
  }  
...
float write_shortcut_layer(layer l, FILE *fp, int index)
{
  int n ;
...
float write_upsample_layer(layer l, FILE *fp, int index)
{
  int n ;
...
float write_yolo_layer(layer l, FILE *fp, int index)
{
  int n ;
  ...
  fprintf(fp, "num=%d\n", l.total);        
  fprintf(fp, "jitter=%f\n", l.jitter);
  fprintf(fp, "scale_x_y=%f\n", l.scale_x_y);
  fprintf(fp, "objectness_smooth=%f\n", l.objectness_smooth);  
  fprintf(fp, "ignore_thresh=%f\n", l.ignore_thresh);
  fprintf(fp, "truth_thresh=%f\n", l.truth_thresh);
  fprintf(fp, "random=%d\n", l.random);
  fprintf(fp, "resize=%f\n", l.resize);
  fprintf(fp, "iou_thresh=%f\n", l.iou_thresh);
  fprintf(fp, "iou_normalizer=%f\n", l.iou_normalizer);
  fprintf(fp, "cls_normalizer=%f\n", l.cls_normalizer);
  fprintf(fp, "obj_normalizer=%f\n", l.obj_normalizer);
  if (l.iou_loss == IOU) {
    fprintf(fp, "iou_loss=iou\n");
  } else if (l.iou_loss == GIOU) {
    fprintf(fp, "iou_loss=giou\n");
  } else if (l.iou_loss == DIOU) {
    fprintf(fp, "iou_loss=diou\n");
  } else if (l.iou_loss == CIOU) {
    fprintf(fp, "iou_loss=ciou\n");
  } else {
    fprintf(fp, "iou_loss=mse\n");
  }
  //NMS

  fprintf(fp, "new_coords=%d\n", l.new_coords);
  if (l.max_delta != FLT_MAX) {
    fprintf(fp, "max_delta=%f\n", l.max_delta);
  }
...
void write_cfg(network net, char *filename)
{
  int i, j, n;
  ...
  fprintf(fp, "learning_rate_min=%f\n", net.learning_rate_min);
  ...
  switch (net.policy) {
  case STEPS:\
    fprintf(fp, "policy=steps\n");
    fprintf(fp, "steps=");
    for(i = 0; i < net.num_steps; ++i){
      fprintf(fp, "%d", net.steps[i]);
      if (n != (net.num_steps - 1)) {
	fprintf(fp, ",");
      }
    }
    fprintf(fp, "\n");
    ...
    break;
  case SGDR:
    fprintf(fp, "policy=sgdr\n");
    fprintf(fp, "sgdr_cycle=%d\n", net.batches_per_cycle);
    fprintf(fp, "sgdr_mult=%d\n", net.batches_cycle_mult);    
  }
    
  
  fprintf(fp, "\n");
  fprintf(fp, "gaussian_noise=%d\n", net.gaussian_noise);
  fprintf(fp, "flip=%d\n", net.flip);    
  ...
  for(i = 0; i < net.n; i++) {
    ...
    if (l.type == CONVOLUTIONAL && l.share_layer == NULL) {
      total_flops += write_conv_layer(l, fp, i);
    } else if (l.type == ROUTE) {
      total_flops += write_route_layer(l, fp, i);
    } else if (l.type == MAXPOOL) {
      total_flops += write_maxpool_layer(l, fp, i);
    } else if (l.type == UPSAMPLE) {
      total_flops += write_upsample_layer(l, fp, i);
    } else if (l.type == SHORTCUT) {
      //Todo
      total_flops += write_shortcut_layer(l, fp, i);
    } else if (l.type == YOLO) {
      total_flops += write_yolo_layer(l, fp, i);
    }
  }
  fprintf(fp, "#Total :%.3fGFLOPS\n", total_flops);
  fclose(fp);
}
