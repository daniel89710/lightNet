...
data load_data_segmentation(int n, char **paths, int m, int w, int h, int c, int out_w, int out_h,  int boxes, int truth_size, int classes, int use_flip, int use_gaussian_noise, int use_blur, int use_mixup,
    float jitter, float resize, float hue, float saturation, float exposure, int mini_batch, int track, int augment_speed, int letter_box, int mosaic_bound, int contrastive, int contrastive_jit_flip, int contrastive_color, int show_imgs)
{
    const int random_index = random_gen();
    c = c ? c : 3;
    if (use_mixup) {
      printf("Strong Augmentations are not supported for Semantic Segmentation\n");
    }
    if (use_mixup == 2 || use_mixup == 4) {
        printf("\n cutmix=1 - isn't supported for Detector (use cutmix=1 only for Classifier) \n");
        if (check_mistakes) getchar();
    ...
    for (i_mixup = 0; i_mixup <= use_mixup; i_mixup++) {
	    ...
	    char labelpath[4096];
	    replace_image_to_mask(filename, labelpath);

            mask = load_image_mat_cv(labelpath, 0);
            if (mask == NULL) {
                printf("\n Error in load_data_detection() - OpenCV \n");
                fflush(stdout);
                if (check_mistakes) {
	    ...
	    pbot = 0;
            ...
            int min_w_h = 0;// = fill_truth_detection(filename, boxes, truth_size, truth, classes, flip, dx, dy, 1. / sx, 1. / sy, w, h);
            //for (int z = 0; z < boxes; ++z) if(truth[z*truth_size] > 0) printf(" track_id = %f \n", truth[z*truth_size + 5]);
            //printf(" truth_size = %d \n", truth_size);
            ...
            image ai = image_data_augmentation(src, w, h, pleft, ptop, swidth, sheight, flip, dhue, dsat, dexp,
                gaussian_noise, blur, boxes, truth_size, truth);
            image am = mask_data_augmentation(mask, out_w, out_h, pleft, ptop, swidth, sheight, flip, 0.0, 1.0, 1.0,
                0, blur, boxes, truth_size, truth);	    

	    //	    save_image_png(ai, "src");
...
data load_data_segmentation(int n, char **paths, int m, int w, int h, int c, int out_w, int out_h, int boxes, int truth_size, int classes, int use_flip, int use_gaussian_noise, int use_blur, int use_mixup,
    float jitter, float resize, float hue, float saturation, float exposure, int mini_batch, int track, int augment_speed, int letter_box, int mosaic_bound, int contrastive, int contrastive_jit_flip, int contrastive_color, int show_imgs)
{
  printf("No support for Segmentation without OpenCV");
  abort();
}
...
data load_data_logistic_regression(int n, char **paths, int m, int w, int h, int c, int out_w, int out_h,  int boxes, int truth_size, int classes, int use_flip, int use_gaussian_noise, int use_blur, int use_mixup,
				   float jitter, float resize, float hue, float saturation, float exposure, int mini_batch, int track, int augment_speed, int letter_box, int mosaic_bound, int contrastive, int contrastive_jit_flip, int contrastive_color, int show_imgs, int scale)
{
    const int random_index = random_gen();
    c = c ? c : 3;
    if (use_mixup) {
    ...
    for (i_mixup = 0; i_mixup <= use_mixup; i_mixup++) {
	    ...
	    pbot = 0;
            ...
            image ai = image_data_augmentation(src, w, h, pleft, ptop, swidth, sheight, flip, dhue, dsat, dexp,
                gaussian_noise, blur, boxes, truth_size, truth);
            image am = get_depth_data_augmentation(mask, out_w, out_h, pleft, ptop, swidth, sheight, flip, 0.0, 1.0, 1.0,
						   0, blur, boxes, truth_size, truth, scale);	    

	    //	    save_image_png(ai, "src");
	    save_image_png(am, "depth");	    
...
void *load_thread(void *ptr)
{
    ...
    if (a.type == OLD_CLASSIFICATION_DATA){
      ...
      *a.d = load_data_segmentation(a.n, a.paths, a.m, a.w, a.h, a.c, a.out_w, a.out_h, a.num_boxes, a.truth_size, a.classes, a.flip, a.gaussian_noise, a.blur, a.mixup, a.jitter, a.resize,
				    a.hue, a.saturation, a.exposure, a.mini_batch, a.track, a.augment_speed, a.letter_box, a.mosaic_bound, a.contrastive, a.contrastive_jit_flip, a.contrastive_color, a.show_imgs);
    } else if (a.type == REGRESSION_DATA){
      *a.d = load_data_logistic_regression(a.n, a.paths, a.m, a.w, a.h, a.c, a.out_w, a.out_h, a.num_boxes, a.truth_size, a.classes, a.flip, a.gaussian_noise, a.blur, a.mixup, a.jitter, a.resize,
					   a.hue, a.saturation, a.exposure, a.mini_batch, a.track, a.augment_speed, a.letter_box, a.mosaic_bound, a.contrastive, a.contrastive_jit_flip, a.contrastive_color, a.show_imgs, a.scale);      
