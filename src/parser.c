...
extern float *get_classes_multipliers(char *cpc, const int classes, const float max_delta);
...
LAYER_TYPE string_to_layer_type(char * type)
{
    ...
    if (strcmp(type, "[logi]")==0
            || strcmp(type, "[logistic]")==0) return LOGXENT;    
...
convolutional_layer parse_convolutional(list *options, size_params params)
{
    ...
    layer.sparse = option_find_int_quiet(options, "sparse", 0);    
    layer.sparse_mask = NULL;
    if (layer.sparse) {
      layer.sparse_mask = (float *)malloc(sizeof(float) * layer.nweights);
#ifdef GPU
      layer.sparse_mask_gpu = NULL;
      layer.sparse_mask_gpu = (float *)cuda_make_array(layer.sparse_mask, layer.nweights);      
#endif
    }
    layer.pruning = option_find_float_quiet(options, "pruning", 0.0);
    layer.quant = option_find_int_quiet(options, "quant", 0);
    layer.max_float = option_find_float_quiet(options, "max_float", 0);    
    return layer;
}
...
softmax_layer parse_softmax(list *options, size_params params)
{
	int groups = option_find_int_quiet(options, "groups", 1);
	int stride = option_find_int_quiet(options, "stride", 1);
	float max_delta = option_find_float_quiet(options, "max_delta", FLT_MAX);   // set 10
	int total = option_find_int(options, "num", 1);		
	char *cpc = option_find_str(options, "counters_per_class", 0);
	float *classes_multipliers = get_classes_multipliers(cpc, total, max_delta);	
	softmax_layer layer = make_softmax_layer(params.batch, params.inputs, groups, stride, classes_multipliers, total);
	...
	char *a = option_find_str(options, "colormap", 0);
	if (a) {
	  layer.map = (int*)malloc(sizeof(int) * 3 * total);
	  int len = strlen(a);
	  int n = 1;
	  int i;
	  for (i = 0; i < len; ++i) {
...
logistic_layer parse_logistic(list *options, size_params params)
{
	int groups = option_find_int_quiet(options, "groups", 1);
	int stride = option_find_int_quiet(options, "stride", 1);
	float max_delta = option_find_float_quiet(options, "max_delta", FLT_MAX);   // set 10
	int total = option_find_int(options, "num", 1);
	int scale = option_find_int(options, "scale", 90);			
	char *cpc = option_find_str(options, "counters_per_class", 0);
	float *classes_multipliers = get_classes_multipliers(cpc, total, max_delta);	
	logistic_layer layer = make_logistic_layer(params.batch, params.inputs, groups, stride, classes_multipliers, total);
	layer.w = params.w;
	layer.h = params.h;
	layer.c = params.c;
	layer.scale = scale;
...
layer parse_shortcut(list *options, size_params params, network net)
{
    ...
    s.max_float = option_find_float_quiet(options, "max_float", 0);    
    return s;
}
...
layer parse_upsample(list *options, size_params params, network net)
{
    ...
    l.max_float = option_find_float_quiet(options, "max_float", 0);        
    return l;
}
...
void save_merge_weights_from_index(network net, network net2, char *filename, int from, int save_ema)
{
#ifdef GPU
    if(net.gpu_index >= 0){
        cuda_set_device(net.gpu_index);
    ...
    for(i = from; i < net2.n; ++i){
        layer l = net2.layers[i];
        if (l.type == CONVOLUTIONAL && l.share_layer == NULL) {
            if (save_ema) {
                save_convolutional_weights_ema(l, fp);
            }
