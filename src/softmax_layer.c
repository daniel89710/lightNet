...
softmax_layer make_softmax_layer(int batch, int inputs, int groups, int stride, float *classes_multipliers, int classes)
{
...
void forward_softmax_layer_gpu(const softmax_layer l, network_state net)
{
    if(l.softmax_tree){
	  ...
	  if (l.groups > 1 && l.stride > 1) {
	    //Pixel-wise Softmax for Semantic Segmentation
	    softmax_gpu_new_api(net.input, l.inputs/l.groups, l.batch, l.inputs, l.groups, 1, l.stride, l.temperature, l.output_gpu);	    
	  } else {
    ...
    if(net.truth && !l.noloss){
	  ...
	  //Pixel-wise Softmax for Semantic Segmentation
	  int b;
	  int chan = l.inputs / l.groups;
	  for (b = 0; b < l.batch; b++) {
	    spatial_softmax_x_ent_gpu(l.inputs/chan, chan, l.stride, &(l.output_gpu[b * l.inputs]), &(net.truth[b * l.stride]), &(l.delta_gpu[b * l.inputs]), &(l.loss_gpu[b * l.inputs]), l.biases_gpu);
	  }
	} else {
	  softmax_x_ent_gpu(l.batch*l.inputs, l.output_gpu, net.truth, l.delta_gpu, l.loss_gpu);
        ...
        l.cost[0] = sum_array(l.loss, l.batch*l.inputs);
	double mean = mean_array(l.loss, l.batch*l.inputs);
	if (l.biases) {
	  int i;
	  int chan = l.inputs / l.groups;
	  for (i = 0; i < chan; i++) {
	    printf("#%d=>%f,", i, l.biases[i]);
	  }
	}	
	printf("softmaxcost %f,%f\n", l.cost[0], mean);
	l.cost[0] = mean;
    }
}
