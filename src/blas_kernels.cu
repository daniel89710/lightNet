...
__global__ void spatial_softmax_x_ent_kernel(int n, int chan, int stride, float *pred, float *truth, float *delta, float *error)
{
	int c;
	int i = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
	if (i <  n) { 
	   int label = (int)(truth[i] * 255.0);
	   for (c = 0; c < chan; c++) {
		float p = pred[i+c*stride];
		float t = (c == label) ? 1.0 : 0.0;
		error[i+c*stride] = -t*log(p+.0000001) - (1.0-t)*log(1-p+.0000001);
		delta[i+c*stride] = t-p;
	   }

	   //printf("label=%d,%f,%f,%f\n", label, pred[i], pred[i+1*stride],pred[i+2*stride]);

	}
}

__global__ void spatial_softmax_x_ent_biases_kernel(int n, int chan, int stride, float *pred, float *truth, float *delta, float *error, float *biases)
{
	int c;
	int i = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
	if (i <  n) { 
	   ...
	   for (c = 0; c < chan; c++) {
		...
		error[i+c*stride] *= biases[label];
		delta[i+c*stride] *= biases[label];		
...
extern "C" void spatial_softmax_x_ent_gpu(int n, int c, int stride, float *pred, float *truth, float *delta, float *error, float *biases)
{
  if (biases) {
    spatial_softmax_x_ent_biases_kernel << <cuda_gridsize(n), BLOCK, 0, get_cuda_stream() >> >(n, c, stride, pred, truth, delta, error, biases);
  } else {
    spatial_softmax_x_ent_kernel << <cuda_gridsize(n), BLOCK, 0, get_cuda_stream() >> >(n, c, stride, pred, truth, delta, error);
  }
  CHECK_CUDA(cudaPeekAtLastError());
}

__global__ void apply_fake_quantization_kernel(int n, float *data, float scale, int max_value)
{

  int i = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
  if (i <  n) {
    int q_d = (int)(data[i] * scale + 0.5);
    q_d = (q_d > max_value) ? max_value : q_d;
    q_d = (q_d < (max_value*(-1))) ? (max_value*(-1)) : q_d;
    data[i] = q_d / scale;      
  }
}

extern "C" void apply_fake_quantization_gpu(int n, float *data, float scale, int max_value)
{

  apply_fake_quantization_kernel<< <cuda_gridsize(n), BLOCK, 0, get_cuda_stream() >> >(n, data, scale, max_value);
...
__global__ void logistic_x_ent_kernel(int n, float *pred, float *truth, float *delta, float *error)
{
	...
	if (t > 0.0) {
          ...
          //error[i] = -t*log(p) - (1-t)*log(1-p+.0000001);
	  //	  error[i] =  - (1-t)*log(1-p+.0000001);
          delta[i] = (t-p);
